...
nobsPerDay <- hoursPerDay * (minutesPerHour / minutesPerInterval)
nobsPerDay
```
...
hist(bins$totalStepsPerDay, breaks=nbins,
     ...
     xlim=c(0, max(bins$totalStepsPerDay)),
     ylab="Number of Days")
...
```{r}
activity %>% group_by(interval) %>% filter(!is.na(steps)) %>%
...
```
Find then interval that averaged the maximum steps per daily.
```{r}
interval %>% filter(avgSteps == max(interval$avgSteps)) -> maxInterval
maxInterval
```
```{r}
maxAvgInterval <- intervalFormat(maxInterval$interval)
```
Create a line plot showing the average steps taken throught the day from 
interval 0:00 (0) through interval (2355) 23:55
<!--
```{r, results='hide'}
library(ggplot2)
```
...
ggplot(data=interval, aes(x=interval, y=avgSteps)) +
    ...
    xlab("Tom of Day (24 hour clock)") +
    ylab("Average Steps per 5 Min Interval per Day") +
    geom_line(aes(x=maxInterval$interval, color=maxAvgInterval))
```
